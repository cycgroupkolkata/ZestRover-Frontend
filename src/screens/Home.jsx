...
import React, { useState } from "react";
import { HomeBg } from "./../assets/imges/bg-images/index";
import { FlightSearchCard } from "../components";
import { GiCommercialAirplane } from "react-icons/gi";
  ...
  return (
    <div className="bg-slate-300">
      ...
      >
        ...
        {/* Hero Section */}
        <div className="text-center sm:text-left p-8 sm:p-12 z-10 max-w-4xl mx-auto">
        ...
        {/* Main Content Wrapper */}
        <div className="text-center mx-4 sm:mx-10 bg-white/90 text-black p-8 sm:p-10 rounded-lg shadow-2xl z-30 relative max-w-3xl w-full mt-12">
          {/* Tabs for Flight and Hotel */}
          <div className="bg-white shadow-xl rounded-md px-8 py-4 flex items-center space-x-8 justify-center mb-8">
            <button
              onClick={() => setIsFlight(true)}
              className={`${
                isFlight ? "text-blue-600" : "text-gray-600"
              } flex items-center space-x-2 py-2 px-4 rounded-lg transition-all duration-300 ease-in-out transform hover:bg-blue-100`}
            >
